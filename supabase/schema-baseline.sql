...
-- NOTA (2026-08-04): "ANEXO 2026-08-03 — La ronda" ROMPE la regla de arriba a
...
-- ──────────────────────────────────────────────────────────────────────────
-- 20260723_saga_routes + 20260723_saga_route_entries_uniques
-- Itinerarios de lectura: rutas curadas con nombre por saga. Aplicadas a prod
-- el 2026-07-22 ANTES de mergear la rama (puramente aditivas, sin dependencia
-- de orden con el despliegue).
-- ──────────────────────────────────────────────────────────────────────────
...
-- ---------------------------------------------------------------
-- Ventanas de colocación de una entrada `libre`
...
-- ---------------------------------------------------------------
-- save_saga_sequence crece a cinco argumentos; la de cuatro pasa a envoltorio
-- (20260727_save_saga_sequence_windows.sql)
-- ---------------------------------------------------------------
-- Fase 2b: `save_saga_sequence` también guarda las ventanas de colocación
-- (`saga_placement_windows`, arriba).
--
...
-- despliegue (ver el ANEXO de arriba: es la migración que todavía no existe).
create or replace function public.save_saga_sequence(
  p_saga_id uuid,
  p_entries jsonb,
  p_blocks  jsonb,
...
-- ---------------------------------------------------------------
-- El curador decide si su saga enseña mapa
-- (20260728_sagas_show_map.sql)
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- Migración de lo que solo sabía el grafo a itinerarios
-- (20260728_migrar_grafos_a_itinerarios.sql)
-- ---------------------------------------------------------------
...
-- ============================================================================
-- ANEXO 2026-08-02 — Social fase 1: identidad canónica de interacciones
-- (aplicado en producción)
-- Ocho migraciones, replicadas ÍNTEGRAS y EN EL ORDEN REAL DE PRODUCCIÓN, que
-- no es el de dev: en prod el fix de la audiencia de checkpoint viajó CON la
-- cadena expansiva (la migración expansiva ya reapunta el SELECT de
-- `comments`/`reactions` a `can_view_interaction_target`, así que el defecto
-- habría estado vivo desde la primera migración), y el CONTRATO entró EL
-- ÚLTIMO, después de que el bundle canónico estuviera desplegado y verificado.
-- En dev el contrato se aplicó antes que ese fix; se conserva el orden de prod
-- porque es el que reproduce producción desde cero.
-- Igual que en el anexo de la fase 0, NO se consolida el DDL superpuesto: las
-- migraciones se replican tal cual y las redefiniciones sucesivas quedan en su
-- sitio, de modo que la última definición de cada objeto es el estado final.
-- Objetos redefinidos más de una vez aquí: `private.can_view_interaction_target`
-- (expansiva → `_security` → `_checkpoint_audience_fix`),
-- `private.sync_checkpoint_interaction_target` (expansiva → `_checkpoint_owner_fix`),
-- `private.resolve_notification_interaction_target` (expansiva →
-- `_notification_writer_fix`), `private.cleanup_social_target` (expansiva →
-- contrato) y las cinco `SECURITY DEFINER` que reescribe el contrato.
-- ============================================================================

-- 20260730212803_social_interaction_targets_expand.sql
...
-- 20260730213248_social_interaction_targets_security.sql
...
-- 20260730214405_social_interaction_targets_compatibility_hardening.sql
...
-- 20260730214621_social_interaction_targets_visibility_acl.sql
...
-- 20260801115944_social_interaction_targets_checkpoint_owner_fix.sql
...
-- 20260801135656_social_interaction_targets_notification_writer_fix.sql
...
-- 20260802013421_social_interaction_targets_checkpoint_audience_fix.sql
...
-- 20260801224621_social_interaction_targets_contract.sql
...
-- ============================================================================
-- ANEXO 2026-08-03 — La ronda: latido semanal de club (spec
-- docs/superpowers/specs/2026-08-03-club-rondas-design.md; decisión de fondo
...
-- añadiendo al mismo fichero lógico según avanzaban: (1) `club_rounds` (tabla
-- + RLS + triggers + los 4 valores de enum), (2) `club_rounds_functions` (las
-- cuatro funciones, con `get_club_round_state` SIN la columna `house_prompt`
-- todavía), (3) `club_rounds_functions_fixes` (mismo día: corrige
-- `ensure_club_round`, que devolvía la ronda existente sin comprobar autoría
-- cuando alguien proponía tarde — `round_already_open` no llegaba a
-- lanzarse), y (4) `club_rounds_house_prompt_column` (`drop function` +
-- `create function` de `get_club_round_state` para añadir esa columna:
-- `create or replace` no puede cambiar la lista de columnas de salida de una
-- función `returns table`, así que hizo falta el DROP). Lo que sigue es la
-- forma FINAL, consolidada, idéntica a `20260803_club_rounds.sql`: aplicarla
-- tal cual a prod, como pieza única, es correcto — prod nunca pasará por la
-- forma intermedia de `get_club_round_state` que obligó al DROP en dev, así
-- que no hace falta reproducir los cuatro pasos, solo entender por qué dev
-- los tiene.
-- ============================================================================
...
-- ═══════════════════════════════════════════════════════════════════════════
-- ANEXO 2026-08-05 — Hitos de buddy_read autodeclarados (issues #470/#471)
-- Dos migraciones aplicadas en dev y prod el 2026-08-05, en este orden.
-- ═══════════════════════════════════════════════════════════════════════════

-- ── 20260826_confirm_checkpoint_lee_passes.sql ──────────────────────────────

-- Arquitectura híbrida, Fase 2: la RPC que reemplaza al snapshot por empuje.
-- El nativo (con su sesión propia de la Fase 1) la llama directamente por
-- PostgREST y recibe EXACTAMENTE el JSON v2 que el parser Kotlin ya consume
-- (WidgetSnapshot.kt) — cambia el transporte, no el contrato.
--
-- SECURITY INVOKER: corre con la RLS del que llama (auth.uid()), así que solo
-- puede ver sus propias filas; el nativo no puede pedir el snapshot de otro.
--
-- Port FIEL de la cadena TS (getTodayFocus + hydrateItems + getProgress +
-- getWeeklyActivity + getStreaks + streak.ts). Tres definiciones de "actividad"
-- que NO se pueden confundir:
--   * racha/semana POR PASE  -> progress_sessions ∪ episode_watches (de ese pase)
--   * racha GLOBAL           -> progress_sessions ∪ passes.finished_on (NO watches)
--   * minutos de HOY         -> progress_sessions.duration_minutes SOLO de libros
-- "Hoy" = Europe/Madrid, la misma convención de día que club_rounds (no UTC).
-- El widget no usa nota/reseña: se omite toda la hidratación de rating/reviews.
...
begin
  ...
  with ip as (
    select p.id pass_id, p.item_type, p.item_id, p.position, p.started_on
    from passes p

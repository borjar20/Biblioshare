-- Precedencia de páginas a DOS peldaños en el snapshot del widget nativo
-- (spec 2026-08-26 §5, Task 12): la edición que el usuario IDENTIFICÓ en su
-- pase manda; sin ella —o si esa edición no trae páginas— las páginas
-- orientativas de la obra (`books.total_pages`). Es la misma regla que
-- `pagesForPass` / `pickEditionPages` en el código web.
--
-- MUERE el peldaño de la «edición primaria» (y con él el de «cualquier edición
-- con total», que solo existía aquí y en pickEditionPages). `is_primary` nunca
-- fue una decisión: lo ponía un trigger sobre la PRIMERA fila que entrara, que
-- con el sync masivo vivo era la primera de hasta 500 filas bajadas de
-- OpenLibrary. Hoy las ediciones solo existen si alguien identificó su tirada,
-- así que ese peldaño solo servía para que el widget enseñara un porcentaje
-- distinto del de la app para el MISMO pase, sin que nada avisara.
--
-- ÚNICO cambio respecto a 20260806_widget_snapshot_edition_pages.sql: el
-- LATERAL de `cat`. El resto del cuerpo va tal cual, a propósito — `create or
-- replace` reescribe la función entera y cualquier deriva aquí sería una
-- regresión silenciosa en el widget.
--
-- OJO: la columna `book_editions.is_primary` y sus triggers NO se tocan (eso
-- es la fase destructiva, Task 16). Aquí solo se deja de leer.
...
begin
  ...
  cat as ( -- catálogo por tipo (solo un join casa por fila)
    select ip.*,
      ...
      b.author,
      -- Páginas desde la edición IDENTIFICADA en el pase y, si no la hay o no
      -- trae total, books.total_pages (`pagesForPass` en SQL).
    ...
    -- Solo la edición del pase: `be.id = ip.edition_id` es la PK, así que hay
    -- como mucho una fila y no hace falta desempatar. Si el pase no fija
    -- edición (edition_id null) el LATERAL no casa y manda b.total_pages.
    left join lateral (
      select be.total_pages
      from book_editions be
      where ip.item_type = 'book'
        and ip.edition_id is not null
        and be.id = ip.edition_id
        and be.book_id = ip.item_id
    ) ed on true
  ),
  computed as (
    select

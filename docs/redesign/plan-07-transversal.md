...
4. ~~**Onboarding**~~ ✅ **HECHO 2026-07-20.** Asistente de 3 pasos + bienvenida (intereses → primeros títulos → gente y clubes), spec en `docs/superpowers/specs/2026-07-20-onboarding-design.md`. No era un restyle: `src/app/onboarding/` eran 132 líneas que **redirigían de largo**, así que se construyó el flujo entero. Una migración (`profiles.interests`, `profiles.onboarded_at`). **Lee su §3 antes de tocar nada del arranque de sesión:** `proxy.ts`, `AppShell` y la acción de registro usaban «tiene perfil» como sinónimo de «está onboardeado», y las tres hubo que separarlas.

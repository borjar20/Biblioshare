...
- [x] **Sagas v2, fase 3 de 4** (2026-07-19, misma spec, §3) — construida y **verificada** (QA de navegador en 10 pasos, incl. regresión de los 2 bugs corregidos en revisión; e2e nuevos `sagas-v2-editor.spec.ts` 3/3 con doble pasada y seed restaurado; 44 unit): editor del grafo en `/saga/[id]/mapa/editar`, con gate duro collaborator+ (anónimo → login; sin rol suficiente → ficha) y dos entradas de acceso: «Editar grafo» desde la pestaña Mapa y «Configurar orden de lectura» desde Info cuando la saga todavía no tiene grafo. Lienzo **React Flow** editable (arrastrar nodos, conectar aristas con tipo principal/opcional/requisito, borrar); panel izquierdo para **añadir título** (reutiliza el `ItemPicker` de clubes, asegurando membresía directa al guardar — invariante grafo⊆miembros) y para **crear/anidar subsagas** (`SagaPicker`, ciclar color de una paleta sin beige, error de ciclo visible en el propio formulario); **inspector de nodo** con label override, selector de subsaga (edita `saga_items` directamente — la subsaga del nodo se sigue **derivando**, nunca se persiste en el nodo del grafo), nivel, `order_no` con aviso de colisión, gestión de conexiones, y quitar del grafo sin tocar la membresía de la saga. **Guardado atómico** vía RPC `save_saga_graph` (`SECURITY DEFINER`, gate de rol interno además del de UI, full-replace transaccional de nodos+aristas — migración `20260719_save_saga_graph_rpc.sql`, **solo en dev** hasta el merge), acompañado de operaciones de membresía con **re-promoción de `is_primary`**, cerrando el TODO ya documentado en `manage-saga-actions` desde fase 1. El borrador es 100% cliente, con «Descartar» real (sin escritura parcial). De paso cierra los dos DEFERs abiertos en fase 2: el viewer pasa a `next/dynamic` (React Flow queda fuera del bundle cuando la saga no tiene grafo), la leyenda gana «Leyendo ahora» con key estable, y se añade el índice `saga_edges(to_node)`.
  - **Deuda asumida a propósito, sigue abierta**: la navegación por teclado del **viewer** (no del editor) sigue sin resolver — el editor sí es operable sin puntero (selección + flechas de React Flow funcionan ahí); las operaciones de membresía no son transaccionales entre sí ni con la RPC de guardado (un reintento del guardado completo es seguro, no deja estado a medias).
- [x] **Sagas v2, fase 3.5 de 5** (2026-07-19, adenda `docs/superpowers/specs/2026-07-19-sagas-v2-fase-3-5-crear-sagas-design.md` al spec principal — **renumera la antigua "fase 4" a fase 5, sin cambio de alcance**) — construida y **verificada** (e2e `e2e/sagas-v2-curacion.spec.ts` 2/2 con doble pasada + QA de navegador): hasta ahora una saga solo nacía "desde abajo" (asignar un ítem existente, o automáticamente desde una colección TMDB); esta fase añade crearlas también **"desde arriba", vacías**. Índice público `/sagas` (`src/app/sagas/page.tsx`, accesible sin sesión) con buscador (reutiliza `search-sagas.ts`), tarjetas de las sagas **raíz** (`parent_saga_id IS NULL`) con sus subsagas como chips de color, botón «Seguir» por tarjeta, y «Nueva saga» (solo collaborator+) que abre `/sagas/nueva` — hoja gated con nombre + acento, `createSaga` → redirect a `/saga/[id]`. En la ficha de saga: botón «Anidar en universo» (`SagaPicker` o crear el universo nuevo en el mismo gesto) y «Editar ficha» (nombre, sinopsis, portada vía service-role, acento, quitar/asignar universo padre) — ambos collaborator+, vía `src/lib/sagas/curation-actions.ts` (`setParentSaga`, `updateSagaMeta`, `uploadSagaCover`). `setParentSaga` resuelve el universo padre con **buscar-o-crear homónimo case-insensitive** entre sagas manuales (mismo patrón que `assignItemToSaga`) y devuelve siempre el **nombre canónico ya en BD** (nunca el tecleado por el cliente), para no colgar de un duplicado si ya existía una saga manual con ese nombre en otra grafía. El bloque de sagas del editor de catálogo (§7.38, `catalog-editor.tsx`) pasa a mostrar **todas** las membresías del ítem como chips (no solo la primary), con **«hacer principal»** por chip (`setPrimarySaga`: des-primaria la actual y promociona la elegida, en ese orden, por el índice único parcial `saga_items_primary_idx`) — cierra el TODO de re-promoción manual que arrastraba `manage-saga-actions.ts` desde fase 1. Enlace «Explorar sagas» añadido desde `/buscar`. **Sin migraciones de BD** (todas las columnas usadas ya existían).
- [x] **Sagas v2, fase 5 de 5** (2026-07-19, misma spec §4) — construida y **verificada** (vitest 58/58, e2e `e2e/sagas-v2-biblioteca.spec.ts` 2/2 con doble pasada, QA de navegador completo): tercera subpestaña **«Sagas»** en Mi Biblioteca (`src/app/coleccion/page.tsx`, junto a `Colecciones | Todo`), con `getFollowedSagas` (`src/lib/sagas/get-followed-sagas.ts`) resolviendo en **consultas planas batched multi-raíz** (BFS de descendientes ≤4 niveles, patrón "nada de una query por saga" de §7.11) y `buildLibrarySagaCards` (`src/lib/sagas/build-library-saga-cards.ts`) **puro** aplicando la regla de cómputo de §1.5: denominador = orden principal (nodos-saga expandidos recursivamente a su propio orden principal), opcionales fuera del cálculo, con dedup entre subsagas. `SagaLibraryCard` (`src/components/library/saga-library-card.tsx`): abanico + tag «◆ Grafo», barra de progreso segmentada por subsaga, bloque «Leyendo ahora» → «Siguiente» → «✓ Completada» (con media propia), orden en curso→no empezadas→completadas, vacío con CTA a `/sagas`. Cierra las 5 fases de la spec `docs/superpowers/specs/2026-07-19-sagas-v2-design.md`.
  - **Resuelta la tensión §1.5 vs §3.5**: el cómputo de §1.5 exigía poder expandir un nodo-saga dentro del orden principal, pero el inspector del editor (§3.5) solo exponía «Orden nº» para nodos-ítem, no para nodos-saga. Se amplió `editor-inspector.tsx` para admitir `order_no` también en nodos-saga — misma regla de opcionalidad que un nodo-ítem (sin `order_no` = rama opcional/spin-off, no una subsaga obligatoria del universo).
  - **El timeline móvil (fase 2, `derive-timeline.ts`) sigue sin pintar nodos-saga** — filtra explícitamente `n.kind === "item"`, exclusión ya documentada al construir fase 2, no tocada aquí: el nuevo `order_no` de un nodo-saga solo alimenta el cómputo de progreso de §1.5 (hero, cards de biblioteca), no la derivación del timeline.
  - **Divergencia aceptada entre hero y card de biblioteca**: el hero de `/saga/[id]` (fase 1) muestra el progreso sobre **todos los miembros** de la saga (directos + de cada subsaga), mientras que la card de esta fase usa solo el **orden principal** (regla de §1.5, la misma que ya aplicaba a Info/timeline). Ambas superficies eran ya así de nacimiento — no es una regresión de esta fase, sino la primera vez que coexisten en pantalla y conviene dejarlo anotado.
- [x] **Sagas v2, mejoras post-v2** (2026-07-19, spec `docs/superpowers/specs/2026-07-19-sagas-mejoras-post-v2-design.md`, lote de 4 sobre las fases 1–5 ya mergeadas) — construidas y **verificadas** (e2e de curación 2/2 con doble pasada + editor `sagas-v2-editor.spec.ts` 3/3 + mapa `sagas-v2-mapa.spec.ts` 4/4 + biblioteca 2/2, QA de navegador 5/5):
  - **Fix móvil**: la cabecera de la pestaña Info (`saga-info.tsx`) ya no desborda a 390px — los hasta tres botones de curación («Editar ficha», «Anidar en universo», «Configurar orden de lectura») pasan a un contenedor propio con `flex-wrap` en vez de compartir fila sin wrap con el título de sección.
  - **Zona de peligro «Borrar saga»** al final de `/saga/[id]/editar` (`src/components/saga/saga-meta-editor.tsx`), confirmación en dos pasos: `deleteSaga(sagaId)` en `src/lib/sagas/curation-actions.ts`, gate collaborator+, con migración nueva `20260719_sagas_delete_policy.sql` — `sagas` tenía RLS con SELECT/INSERT/UPDATE pero **ninguna policy de DELETE**, así que el `DELETE` de la action afectaba 0 filas en silencio (hallazgo de revisión). Redirect a `/sagas` tras borrar. El e2e de curación se autolimpia ahora vía esta zona de peligro (cierra el DEFER 9 del PR #90).
  - **«Sacar del universo» por subsaga** desde el panel izquierdo del editor del grafo (`editor-left-panel.tsx`/`saga-graph-editor.tsx`): acción **inmediata** (no forma parte del borrador) que llama a `setParentSaga(childId, null)` y, en el mismo gesto, quita del borrador el nodo (y sus aristas) de esa subsaga si lo tenía — evita el nodo huérfano cuya expansión quedaría vacía. Lleva un registro explícito de subsagas des-anidadas en la sesión para que «Descartar» solo excluya esos nodos y no toque nodos de otras sagas anidadas profundas.
  - **Aristas flotantes con conexión desde los 4 lados** (`ConnectionMode.Loose` de React Flow): cada nodo gana puntos de conexión en los 4 lados (`editor-node.tsx`) y las aristas se dibujan automáticamente entre los lados más cercanos de ambos nodos (`src/components/saga/graph/floating-edge.tsx`, patrón floating-edges — intersección de la línea entre centros con el rect de cada nodo). El viewer (`saga-graph-view.tsx`) reutiliza el mismo componente para que mapa y editor se vean igual; los estilos por `edge_type` (principal/opcional/requisito) se conservan. El lado elegido **no se persiste** — `saga_edges` no cambia de esquema, sin migración.
...
### 7.7 Importar biblioteca desde Goodreads / Letterboxd — *hecho*

> **Actualizado 2026-07-20: Bookmory RETIRADO.** Era la única fuente en `.xlsx`, y
> sostenía sola la dependencia `exceljs`. Al quitarla se fue también el envoltorio
> `FormatDetection` (con solo CSV, su campo `kind` sobraba). El importador queda en
> **dos fuentes, ambas CSV**. Lo de abajo describe el alcance original de julio y se
> conserva como registro; para el estado real manda este aviso.
- [x] Página `/importar` (`src/app/importar/{page,actions,import-form,unmatched-row-form}.tsx`): sube un fichero exportado de **Goodreads** (CSV), **Letterboxd** (`diary.csv`) o **Bookmory** (`.xlsx`), lo parsea, matchea cada fila contra el catálogo/APIs externas y crea los pases de golpe (`passes`; **no** `library_entries`, congelada desde el hub de pases), reutilizando `findOrCreateCatalogItem`. Formato autodetectado en `src/lib/import/detect-format.ts`; parsers dedicados por fuente (`parse-goodreads.ts`, `parse-letterboxd.ts`, `parse-bookmory.ts`, todos con salida a un `types.ts` común); matching en `match-row.ts`; alta/actualización de fila en `commit-row.ts`. Dependencias nuevas: `papaparse` (CSV) y `xlsx` (Bookmory). `next.config.ts` sube `experimental.serverActions.bodySizeLimit` a `5mb` para admitir CSVs/XLSX grandes.
...
- **Actualización (2026-07-19, Sagas v2 fase 3 de 4, ver 7.4)**: el grafo deja de poblarse solo por SQL — editor en `/saga/[id]/mapa/editar`, gateado a collaborator+, con lienzo React Flow editable (nodos/aristas/subsagas) y guardado atómico vía RPC `save_saga_graph` (full-replace transaccional). El selector de subsaga del inspector de nodo **edita `saga_items`, no el nodo**: la subsaga de un nodo sigue siendo, como se previó en fase 1, un valor derivado, nunca guardado en `saga_nodes`. De paso se cierran los dos DEFERs de fase 2 (viewer con `next/dynamic`, leyenda con «Leyendo ahora», índice `saga_edges(to_node)`) y el TODO de re-promoción de `is_primary` que arrastraba `manage-saga-actions.ts` desde fase 1. La deuda de teclado de fase 2 queda acotada al **viewer**: el editor sí es operable sin puntero. Sigue pendiente solo la fase 4 (pestaña Sagas de Mi Biblioteca).
- **Actualización (2026-07-19, Sagas v2 fase 3.5 de 5, ver 7.4)**: hasta esta fase una saga solo nacía "desde abajo" (asignación de un ítem o alta automática desde TMDB); ahora también nace "desde arriba", vacía, vía el índice público `/sagas` y `/sagas/nueva`. El universo padre se resuelve con el mismo patrón buscar-o-crear homónimo case-insensitive que `assignItemToSaga`, y la ficha de saga gana edición completa de metadatos (nombre, sinopsis, portada, acento, padre). El multi-chip de sagas del editor de catálogo gana «hacer principal», cerrando el TODO de re-promoción manual abierto desde fase 1. La antigua fase 4 (pestaña Sagas de Mi Biblioteca) se renumera a **fase 5**, sin cambio de alcance.
- **Actualización (2026-07-19, Sagas v2 mejoras post-v2, ver 7.4)**: lote de 4 mejoras sobre las 5 fases ya mergeadas, sin nuevo alcance de spec. `sagas` gana su primera **policy `DELETE`** (`20260719_sagas_delete_policy.sql`, collaborator+) — no existía ninguna hasta ahora; borrar una saga **no cascada a subsagas**, que sobreviven como raíces (`parent_saga_id on delete set null`), y aplica también a sagas TMDB (una TMDB borrada puede reaparecer por cache-as-you-go, aceptado). «Sacar del universo» por subsaga se añade al panel del editor como **acción inmediata y persistida** (no parte del borrador de guardado), con limpieza del nodo del borrador en el mismo gesto. El grafo gana aristas flotantes desde los 4 lados del nodo (`ConnectionMode.Loose`); el **lado de conexión no se persiste** — se recalcula siempre por ruta automática entre los rects de los nodos, sin tocar el esquema de `saga_edges`.
...
| 2026-07-19 | **Sagas v2, fase 3 de 4** (§7.4/§7.34) construida y **verificada** (QA navegador 10 pasos, incl. regresión de los 2 bugs corregidos en revisión; e2e nuevos `sagas-v2-editor.spec.ts` 3/3 con doble pasada y seed restaurado; 44 unit) — editor del grafo en `/saga/[id]/mapa/editar`, gate duro collaborator+ (anónimo→login, sin rol→ficha); lienzo React Flow editable (nodos, aristas tipadas principal/opcional/requisito, borrado); panel de añadir título (`ItemPicker` de clubes, asegura membresía directa al guardar) y de subsagas (`SagaPicker`, ciclar color, error de ciclo visible); inspector de nodo (label override, selector de subsaga que edita `saga_items` sin persistir la subsaga en el nodo, nivel, `order_no` con aviso de colisión, conexiones, quitar del grafo sin tocar membresía); guardado atómico vía RPC `save_saga_graph` (migración `20260719_save_saga_graph_rpc.sql`, solo en dev hasta el merge); cierra los DEFERs de fase 2 (`next/dynamic`, leyenda «Leyendo ahora», índice `saga_edges(to_node)`) | Ver `docs/superpowers/specs/2026-07-19-sagas-v2-design.md` §3. Decisiones reales: el guardado del grafo se hace con una **RPC `SECURITY DEFINER` de full-replace transaccional** (`save_saga_graph`), no con escrituras incrementales desde el cliente — así el gate de rol se aplica también dentro de la función (no solo en la UI) y un reintento tras fallo de red no puede dejar el grafo a medias; las operaciones de membresía que acompañan al guardado (alta de miembro directo, re-promoción de `is_primary`) sí van aparte, sin envolverlas en la misma transacción, porque un reintento del guardado completo ya es seguro por sí solo. La subsaga de un nodo se confirma como **dato derivado de `saga_items`**, nunca persistido en `saga_nodes` — el inspector edita la membresía, no el nodo, cerrando así la previsión de fase 1 de que grafo e Info nunca discrepen. La deuda de teclado de fase 2 se acota explícitamente al viewer: el editor sí es operable sin puntero (selección + flechas de React Flow). Cierra también el TODO de re-promoción de `is_primary` documentado en `manage-saga-actions.ts` desde fase 1. Fase 4 (pestaña Sagas de Mi Biblioteca) sigue pendiente |
| 2026-07-19 | **Sagas v2, fase 3.5 de 5** (§7.4/§7.34) construida y **verificada** (e2e `e2e/sagas-v2-curacion.spec.ts` 2/2 con doble pasada + QA de navegador) — adenda al spec principal, aprobada en brainstorming el mismo día: índice público `/sagas` (buscador, tarjetas de sagas raíz con subsagas como chips, «Seguir», «Nueva saga» collaborator+ → `/sagas/nueva`); botones «Editar ficha»/«Anidar en universo» en `/saga/[id]` (`src/lib/sagas/curation-actions.ts`: `createSaga`, `setParentSaga`, `updateSagaMeta`, `uploadSagaCover`); multi-chip de sagas con «hacer principal» (`setPrimarySaga`) en el editor de catálogo de las tres fichas; enlace «Explorar sagas» desde `/buscar`. Sin migraciones | Ver `docs/superpowers/specs/2026-07-19-sagas-v2-fase-3-5-crear-sagas-design.md`. Decisiones reales: **sagas nacen también "desde arriba"** — hasta ahora una saga solo nacía al asignar un ítem existente o automáticamente desde una colección TMDB; esta fase añade crearlas vacías desde el índice, invirtiendo el orden habitual ("creo el contenedor y luego lo relleno" en vez de "curo un ítem y de paso nace la saga"). **Universo padre editable con buscar-o-crear homónimo case-insensitive**: `setParentSaga` reutiliza una saga manual existente con el mismo nombre (sin distinguir mayúsculas) en vez de crear un duplicado, mismo patrón que `assignItemToSaga` (fase 1), y devuelve siempre el **nombre canónico ya en BD** al cliente, nunca el tecleado, porque la rama "reutilizar homónima" puede diferir en grafía de lo que el formulario tenía a mano. **La policy `UPDATE` de `sagas` se deja sin endurecer** (`authenticated`, `using(true)`) pese a que esta fase añade edición de metadatos real: la necesita el cache-as-you-go de colecciones TMDB, que escribe en `sagas` como cualquier autenticado; el gate de collaborator+ para curación manual (crear, anidar, editar ficha, hacer principal) se aplica en la capa de server action (`requireCollaborator`), mismo criterio que §8-H (RBAC en capa de app, no en RLS de tablas compartidas). **«Hacer principal» reemplaza a la re-promoción manual**: antes solo se podía quitar una membresía y re-crearla como primary desde el formulario de asignar; ahora `setPrimarySaga` hace el des-primariar→promocionar en un solo gesto, respetando el orden que exige el índice único parcial `saga_items_primary_idx`. La antigua fase 4 (pestaña Sagas de Mi Biblioteca) se renumera a fase 5, sin cambio de alcance |
| 2026-07-19 | **Sagas v2, fase 5 de 5** (§7.4) construida y **verificada** (vitest 58/58, e2e `e2e/sagas-v2-biblioteca.spec.ts` 2/2 con doble pasada, QA de navegador completo) — cierra la spec: tercera subpestaña «Sagas» en Mi Biblioteca, `getFollowedSagas` (consultas planas batched multi-raíz, BFS ≤4 niveles) y `buildLibrarySagaCards` puro (regla de cómputo de §1.5: orden principal, nodos-saga expandidos recursivamente, opcionales fuera, dedup entre subsagas); cards con abanico/tag Grafo, barra segmentada por subsaga, bloque Leyendo ahora→Siguiente→Completada con media propia, orden en curso→no empezadas→completadas, vacío con CTA a `/sagas` | Ver `docs/superpowers/specs/2026-07-19-sagas-v2-design.md` §4. Decisiones reales: **la tensión §1.5 vs §3.5 se resolvió admitiendo `order_no` también en nodos-saga** dentro del inspector del editor (antes solo lo tenían los nodos-ítem) — la ausencia de `order_no` sigue significando "rama opcional/spin-off", ahora también para una subsaga entera, sin flag aparte, coherente con la regla de opcionalidad ya fijada en fase 1. **El timeline móvil (fase 2, `derive-timeline.ts`) queda explícitamente fuera de este `order_no` nuevo**: sigue filtrando solo nodos-ítem (`n.kind === "item"`), exclusión ya documentada al construir fase 2 y no revisada aquí — el `order_no` de un nodo-saga alimenta el cómputo de progreso de §1.5 (hero, Info, cards de biblioteca), nunca la derivación del timeline ramificado. **Divergencia aceptada entre superficies**: el hero de la ficha de saga (fase 1) computa progreso sobre *todos* los miembros (directos + de cada subsaga), mientras que la card de biblioteca de esta fase usa solo el *orden principal* (regla de §1.5 ya vigente desde Info/timeline) — ambas superficies nacieron así por separado; esta fase es la primera vez que conviven en pantalla, así que se deja constancia explícita de que no es una regresión sino un criterio ya existente aplicado de forma consistente |
| 2026-07-19 | **Sagas v2, mejoras post-v2** (§7.4/§7.34) construidas y **verificadas** (e2e de curación 2/2 con doble pasada + editor 3/3 + mapa 4/4 + biblioteca 2/2, QA de navegador 5/5) — lote de 4 sobre las fases 1–5 ya mergeadas: fix del descuadre móvil de la cabecera de Info (`flex-wrap` en el contenedor de botones de curación); zona de peligro «Borrar saga» en `/saga/[id]/editar` (`deleteSaga`, migración `20260719_sagas_delete_policy.sql`); «Sacar del universo» por subsaga desde el panel del editor (acción inmediata, con limpieza del nodo del borrador y registro de des-anidadas para que «Descartar» no toque nodos de sagas anidadas profundas); aristas flotantes con conexión desde los 4 lados del nodo en viewer+editor (`ConnectionMode.Loose`, `floating-edge.tsx`) | Ver `docs/superpowers/specs/2026-07-19-sagas-mejoras-post-v2-design.md`. Decisiones reales: **borrar una saga no cascada a subsagas** — quedan como sagas raíz (`parent_saga_id on delete set null`), y la acción **vale también para sagas TMDB** (decisión explícita del usuario; una TMDB borrada puede reaparecer por cache-as-you-go, aceptado) — hasta esta mejora `sagas` no tenía ninguna policy `DELETE`, así que el borrado habría afectado 0 filas en silencio. **El lado de conexión de una arista no se persiste**: se resuelve siempre por ruta automática (intersección de la línea entre centros con el rect de cada nodo), sin ampliar el esquema de `saga_edges`. **Des-anidar es una acción inmediata y persistida**, no parte del borrador del editor — a diferencia de añadir/mover nodos o aristas (que sí esperan a «Guardar»), sacar una subsaga del universo escribe al instante vía `setParentSaga`, con la única precaución de no dejar «Descartar» resucitando el nodo huérfano que esa misma acción quitó del borrador |

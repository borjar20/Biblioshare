...
- [x] **Sagas v2, fase 3 de 4** (2026-07-19, misma spec, §3) — construida y **verificada** (QA de navegador en 10 pasos, incl. regresión de los 2 bugs corregidos en revisión; e2e nuevos `sagas-v2-editor.spec.ts` 3/3 con doble pasada y seed restaurado; 44 unit): editor del grafo en `/saga/[id]/mapa/editar`, con gate duro collaborator+ (anónimo → login; sin rol suficiente → ficha) y dos entradas de acceso: «Editar grafo» desde la pestaña Mapa y «Configurar orden de lectura» desde Info cuando la saga todavía no tiene grafo. Lienzo **React Flow** editable (arrastrar nodos, conectar aristas con tipo principal/opcional/requisito, borrar); panel izquierdo para **añadir título** (reutiliza el `ItemPicker` de clubes, asegurando membresía directa al guardar — invariante grafo⊆miembros) y para **crear/anidar subsagas** (`SagaPicker`, ciclar color de una paleta sin beige, error de ciclo visible en el propio formulario); **inspector de nodo** con label override, selector de subsaga (edita `saga_items` directamente — la subsaga del nodo se sigue **derivando**, nunca se persiste en el nodo del grafo), nivel, `order_no` con aviso de colisión, gestión de conexiones, y quitar del grafo sin tocar la membresía de la saga. **Guardado atómico** vía RPC `save_saga_graph` (`SECURITY DEFINER`, gate de rol interno además del de UI, full-replace transaccional de nodos+aristas — migración `20260719_save_saga_graph_rpc.sql`, **solo en dev** hasta el merge), acompañado de operaciones de membresía con **re-promoción de `is_primary`**, cerrando el TODO ya documentado en `manage-saga-actions` desde fase 1. El borrador es 100% cliente, con «Descartar» real (sin escritura parcial). De paso cierra los dos DEFERs abiertos en fase 2: el viewer pasa a `next/dynamic` (React Flow queda fuera del bundle cuando la saga no tiene grafo), la leyenda gana «Leyendo ahora» con key estable, y se añade el índice `saga_edges(to_node)`.
  ...
  - **Pendiente (fase 4, misma spec)**: pestaña «Sagas» en Mi Biblioteca (`getFollowedSagas`, follow/unfollow desde la lista de sagas seguidas) — el follow en sí (`saga_follows`) ya existe desde fase 1, solo falta esta superficie de listado.

...
8. **Aristas de cadena**: entre huecos consecutivos **del mismo bloque**, de tipo `principal` y con el `accent` del origen. Con tándem, se conecta **cada** nodo del hueco con **cada** nodo del siguiente: un tándem significa «las dos aquí», así que las dos continúan. `id` = `` `chain:${source}->${target}` ``.

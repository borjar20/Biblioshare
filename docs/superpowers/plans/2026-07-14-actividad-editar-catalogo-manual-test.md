...
Tres cambios en las vistas de actividad de club: la curación del pool se separa a una
vista "Modificar actividad", las opiniones viven dentro de cada fila de ítem, y el
picker de ítems busca también en el catálogo general (no solo en tu biblioteca).
Revisar en móvil y escritorio, claro y oscuro.

## Vista "Modificar actividad"

- [ ] En el detalle de una actividad con pool, la cabecera "ÍTEMS" muestra "MODIFICAR ACTIVIDAD" a la derecha **solo** para quien puede curar (list_challenge/tierlist → creador o mod+; buddy_read → cualquier participante).
...
- [ ] Un miembro sin permiso de curar no ve el botón; en un reto de lista sigue viendo la nota "solo quien propuso el reto…" dentro de la vista de edición (si entra como curador).

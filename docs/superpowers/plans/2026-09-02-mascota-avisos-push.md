...
- [ ] **Step 4:** Al día siguiente a las 20:05 Madrid: `select * from pet_nudges order by created_at desc limit 5` en prod y `select status, response from net._http_response order by created desc limit 3` para ver el 200 de la ruta. No hay filas en `pet_nudges` es OK **solo si** `net._http_response` muestra un 200 para `/api/cron/pet-nudges` sobre las 20:00; un 4xx/5xx ahí, o ninguna fila en `net._http_response`, sí es fallo (un PGRST202 en el claim parecería «no hay candidatos» si no se mira esto). El controlador ya verificó que `POST /rest/v1/rpc/claim_pet_nudges` con `{}` devuelve `[]` 200 en dev con service-role y 401 con anon. Anotar en la issue #1014 antes de cerrarla.

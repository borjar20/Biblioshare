...
### Sagas
- [ ] **Orden unificado de sagas, fase 2b de 3: ventanas de las entradas `libre`** — `saga_placement_windows` (tabla nueva, no existe todavía) y la curación de «cuándo se puede leer» para lo que esté `libre`. **Ya no está aplazada por falta de datos**: al curar el Cosmere el 2026-07-26 aparecieron **dos entradas `libre`**, y las dos son **bloques-subsaga** (*Nacidos Era 2* y *Novelas secretas*); `saga_items` sigue con 0 filas `libre`. Eso invirtió su alcance — el sujeto de una ventana no puede ser «solo obra», porque el caso real y el que motiva la fase entera son bloques —, y la deja **dependiendo de la #198 de forma bloqueante**: la ficha tiene que leer la colocación de un bloque y tener dónde pintarlo. Spec propio, ya corregido: `docs/superpowers/specs/2026-07-26-sagas-fase-2b-ventanas-design.md`.
- [ ] **Orden unificado de sagas, fase 3 de 3: retirada de `saga_nodes`/`saga_edges`/`save_saga_graph`** — migrar los cuatro grafos existentes (Cosmere, Mundodisco, Maasverse y el resto) a itinerarios, `DROP` de `save_saga_graph` y de las tablas de grafo, y renombrado/limpieza de `src/lib/sagas/main-order.ts` (solo ordenación para pintar desde la fase 1). Incluye decidir el destino de `apply-membership-ops.ts` (issue #197) y la curación manual de las 12 sagas hijas hoy "sin clasificar" (issue #196) — aunque esa curación ya no está bloqueada por esta fase: el editor de la fase 2a permite hacerla en cualquier momento.

...
- [x] Mascota RPG R2 — implementación técnica del combate mínimo universal (2026-09-06): entrenamiento en `/mascota`, kit genérico para las seis clases, simulación local y validación en servidor, pausa, velocidad y replay (`src/lib/pet/training/`, `src/components/pet/training/`). Verificado con unitarios, TypeScript, build de producción y Playwright con dos cuentas. Sin recompensa ni consumo. Contratos R1 en `src/lib/pet/battle/` y `pet_battles`; sin migración nueva.
- [x] Mascota RPG R2 — legibilidad de la habilidad tras la prueba del usuario (#1082, 2026-09-06): activación manual, efecto y recarga explícitos, avisos de carga/guardia con duración y resultado de la última habilidad fuera del log. Los avisos, el contador y las etiquetas reservan espacio para mantener fijo el botón al cambiar de estado. Integrado en #1100.
- [x] Mascota RPG R2 — aceptado por el usuario para avanzar el 2026-09-06 tras probar los ajustes de #1100 (#1082). No se afirma un número de partidas no comunicado. R3 es el hito activo (#1106); los hitos R no renumeran las fases 1–3 ya implementadas.
- [ ] Mascota RPG R3: implementada ulti con minijuego A, segundo enemigo y 84 animaciones de combate; pendiente aceptación jugable del usuario (#1106).
- [x] Mascota RPG R4a: aventuras derivadas del día con actividad (ventana de siete días), cadena de tres tramos con la vida arrastrada, reintento y reanudación, botín guardado «pendiente de activar», motor r4.1 (cierra #1086, #1087 y #1093). Implementada 2026-09-07 (rama `claude/mascota-features-r4-ef8885`, PR pendiente); calibración: cadena de 3 tramos (política de referencia `interrupt_ulti`, banda 50–75 % / techo `never` 3 %, #1117). Verificada en dev (migración, matriz SQL, unitarios, e2e); tras la aceptación jugable de R3 (#1106), aplicar la migración `20260908_pet_adventures.sql` en prod ANTES del despliegue del código (es aditiva) y solo después mergear. Spec `docs/superpowers/specs/2026-09-06-mascota-r4a-aventuras-design.md`.

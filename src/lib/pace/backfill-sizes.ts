  ...
  await Promise.all([
    ...moviesToFill.map(async (movie) => {
      try {
        ...
        const { error } = await supabase
          ...
          .eq("id", movie.id);
        if (error) console.error("backfillSizes: movie update failed", { id: movie.id, error });
      } catch (error) {
        console.error("backfillSizes: movie failed", { id: movie.id, error });
      }
    }),
    ...seriesToFill.map(async (item) => {
      try {
        ...
        if (error) console.error("backfillSizes: series update failed", { id: item.id, error });
      } catch (error) {
        console.error("backfillSizes: series failed", { id: item.id, error });
      }
    }),
  ]);
}

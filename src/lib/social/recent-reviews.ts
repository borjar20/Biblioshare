...
export async function getRecentReviews(
  ...
  // id/item_type/item_id/updated_at — nunca vienen null en la práctica.
  // updated_at (no created_at, #345) es la hora REAL del terminado: un pase se
  // crea al añadir la obra y el "terminado" llega después como UPDATE. Va en la
  // MISMA lista y no en un `??` a finished_on: `sortDate` promete un timestamp
  // real, y un valor date-only ahí ordena por debajo de todo evento con hora de
  // su día y empata con sus iguales (desempate por uuid). Sin hora real, la fila
  // se descarta igual que sin id.
  ...
  for (const r of diaryRows) {
    ...
    events.push({
      ...
      // sortDate = updated_at (#345), el contrato del campo en FeedEvent: la hora
      // REAL de registro del terminado, igual que el feed. Aquí no ordena (esta
      // lista ordena por eventDate), pero el evento viaja a las mismas tarjetas
      // que el feed, así que nunca puede ser date-only (ver el narrowing de
      // `diaryRows`).

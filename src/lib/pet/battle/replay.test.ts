...
describe("retained battle releases (R5)", () => {
  ...
  it("replays an old record after adding changed rules and enemy content", async () => {
    ...
    const next = await replayBattle({ ...record, snapshot:{...record.snapshot,equipment:{weapon:null,amulet:null}}, rulesetVersion: ruleset.version, contentHash, inputs: [], result: null }, releases);

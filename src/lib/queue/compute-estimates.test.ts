...
import { computeQueueEstimates } from "./compute-estimates";
...
describe("computeQueueEstimates", () => {
  it("estimates a series deterministically from episodes × episode runtime (no pace needed)", () => {
    ...
    const result = computeQueueEstimates([item], null, NO_MOVIE_PACE);
  ...
  it("excludes unresolved items from the total but still totals the resolved ones", () => {
    ...
    const result = computeQueueEstimates(items, bookPace, NO_MOVIE_PACE);

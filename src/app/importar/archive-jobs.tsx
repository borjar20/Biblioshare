...
import { ArchiveCandidateCards } from "./archive-candidate-cards";
import { ArchiveRecoveryReview } from "./archive-recovery-review";
...
export async function ArchiveJobs({ client, userId, page = 0 }: { page?: number; client: Awaited<ReturnType<typeof createClient>>; userId: string }) {
  ...
  return <section className="flex flex-col gap-4" aria-label={t("jobs")}>
    ...
    {jobs.slice(0, 20).map(async (job) => {
      const first = await client.from("archive_import_items").select("ordinal,payload,state,message,candidates,item_id,attempts").eq("job_id", job.id).order("ordinal").range(0, 999);
      if (first.error) return <p key={job.id} role="alert">{t("jobsUnavailable")}</p>;
      const rows = [...first.data];
      for (let offset = 1000; rows.length === offset; offset += 1000) {
        const next = await client.from("archive_import_items").select("ordinal,payload,state,message,candidates,item_id,attempts").eq("job_id", job.id).order("ordinal").range(offset, offset + 999);
      ...
      const totals = await client.rpc("archive_summary", { p_job: job.id });
      const applied = totals.data as { passes: number; planned: number; catalogIncomplete: number } | null;
      const unresolved = (applied?.catalogIncomplete ?? 0) + rows.filter((r) => ["conflict", "ambiguous", "unmatched", "error"].includes(r.state)).length + rows.reduce((count, r) => count + ((r.payload as unknown as ArchiveMovie).reviewConflicts?.length ?? 0), 0);
      ...
      return <article key={job.id} className="flex flex-col gap-3 rounded-card border border-border p-4">
        <h3 className="font-semibold">{t(`states.${job.state === "done" && unresolved > 0 ? "partial" : job.state}`)} · {job.created_at.slice(0, 10)}</h3>
        <p>{t("progress", { imported, total: rows.length })}</p>
        {applied && <p>{t("appliedSummary", applied)}</p>}
        <p>{t("jobSummary", { passes: analysis.summary?.passes ?? 0, planned: analysis.summary?.planned ?? 0, unresolved })}</p>
        <p>{t("dismissedCount", { count: rows.filter(r => r.state === "dismissed").length })}</p>
        {job.state !== "draft" && job.state !== "undone" && <ArchiveRecoveryReview jobId={job.id} />}
        ...
        <details><summary>{t("results")}</summary><ul className="flex flex-col gap-4">{rows.map((row) => {
          ...
          return <li key={row.ordinal} className="flex flex-col gap-2 border-t border-border pt-2">
            ...
            {row.state === "error" && <p>{t(`errorCauses.${["provider_temporary", "provider_missing", "provider_configuration", "temporary", "invalid_metadata", "unknown"].includes(row.message ?? "") ? row.message : "historical"}`)}</p>}
            {job.state !== "draft" && job.state !== "undone" && <>
              {row.state === "ambiguous" && <ArchiveCandidateCards jobId={job.id} ordinal={row.ordinal} candidates={row.candidates as unknown as ImportCandidate[]} />}
              ...
              {row.state === "conflict" && row.message !== "undoLocalChanges" && <p>{t("reviewAbove")}</p>}
              {row.state === "error" && row.attempts < 3 && !["provider_missing", "provider_configuration", "invalid_metadata"].includes(row.message ?? "") && <ArchiveActionForm action={resolveArchive}>{fields}<Button name="decision" value="retry">{t("retry")}</Button></ArchiveActionForm>}
              {row.state === "error" && row.attempts >= 3 && <p>{t("retryLimit")}</p>}

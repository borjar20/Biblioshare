...
5. Clean up any library/catalog data you created for `devtest` via `mcp__supabase-dev__execute_sql` (same pattern used throughout this project: delete `library_entries`/`diary_entries` rows, then any `books`/`movies`/`series` rows you added, scoped by id — never a blanket delete).
